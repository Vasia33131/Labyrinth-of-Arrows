...
namespace Unpuzzle
{
    ...
    public class ScreenLayoutBuilder : MonoBehaviour
    {
        ...
        public void Apply()
        {
            ...
            if (Application.isPlaying || !applyRuntimeLayout) return;
        ...
        private void LayoutBottomHud()
        {
            ...
            float zoneH = Mathf.Max(1f, bottomHud.rect.height);
            float size = config.hudBottomButtonSize;
            if (zoneH > 2f) size = Mathf.Min(size, Mathf.Max(48f, zoneH - 8f));
            float gap = config.hudBottomButtonGap;
            float step = size + gap;

            PlaceHudControl(hintButton, new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(-1.5f * step, 0f), new Vector2(size, size));
            PlaceHudControl(undoButton, new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(-0.5f * step, 0f), new Vector2(size, size));
            PlaceHudControl(extraMoveButton, new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f * step, 0f), new Vector2(size, size));
            PlaceHudControl(restartButton, new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(1.5f * step, 0f), new Vector2(size, size));
        }
        ...
        private void ResolveHudHierarchy()
        {
            ...
            hintButton = Adopt(bottomHud, hintButton, "HintButton");
            undoButton = Adopt(bottomHud, undoButton, "UndoButton");
            extraMoveButton = Adopt(bottomHud, extraMoveButton, "ExtraMoveButton");
        ...
        {
            return name == "WinPanel" || name == "LosePanel"
                || name == "SettingsOverlay" || name == "SettingsPanel"
                || name == "ShopOverlay"
