...
namespace Unpuzzle
{
    ...
    public class ShopPanel : MonoBehaviour
    {
        ...
        /// <summary>Открыть скин без оплаты и надеть. После rewarded.</summary>
        public void UnlockSkin(string id)
        {
            CharacterSkins.UnlockAndEquip(id);
        ...
        public void RefreshSkinSlots()
        {
            ...
            for (int i = 0; i < skins.Count; i++)
            {
                ...
                slot.Bind(skin, CharacterSkins.IsUnlocked(id), CharacterSkins.IsSelected(id),
                    () => HandleSkinClicked(id), RewardedAds.IsBusy);
            }
        }
        ...
        private void PlaceSkinListRect()
        {
            ...
            var rt = skinList as RectTransform;
            if (rt == null) return;
            ...
            var layout = skinList.GetComponent<GridLayoutGroup>();
            if (layout == null) return;
            layout.padding = new RectOffset(8, 8, 18, 20);
            layout.cellSize = new Vector2(400f, 460f);
        ...
        {
            if (characterPage == null || skinList == null) return;

            var pageRt = characterPage.transform as RectTransform;
            var contentRt = skinList as RectTransform;
            if (pageRt == null || contentRt == null) return;

            var image = characterPage.GetComponent<Image>();
            if (image == null) image = characterPage.AddComponent<Image>();
            image.color = new Color(1f, 1f, 1f, 0f);
            image.raycastTarget = true;

            var mask = characterPage.GetComponent<RectMask2D>();
            if (mask == null) mask = characterPage.AddComponent<RectMask2D>();
            mask.padding = new Vector4(0f, 36f, 0f, 0f);

            if (shopMessage != null) shopMessage.maskable = false;

            var scroll = characterPage.GetComponent<ScrollRect>();
            if (scroll == null) scroll = characterPage.AddComponent<ScrollRect>();
            ...
            var fitter = skinList.GetComponent<ContentSizeFitter>();
            if (fitter == null) fitter = skinList.gameObject.AddComponent<ContentSizeFitter>();
        ...
        private void ApplyShopLayout()
        {
            ...
            if (cardRt != null)
            {
                ...
                cardRt.sizeDelta = new Vector2(1060f, 1060f);
            ...
            if (closeXButton != null)
            {
                PlaceRect(closeXButton.transform as RectTransform, new Vector2(1f, 1f), new Vector2(1f, 1f),
                    new Vector2(1f, 1f), new Vector2(-166f, -166f), new Vector2(75f, 75f));
                HideCloseXLabel();
            }

            Transform tabs = card.Find("Tabs");
            PlaceRect(tabs as RectTransform, new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
                new Vector2(0.5f, 1f), new Vector2(0f, -348f), new Vector2(844f, 112f));
            LayoutTabs();
        ...
        private void PlaceBackgroundListRect()
        {
            ...
            var rt = backgroundList as RectTransform;
            if (rt == null) return;
            rt.anchorMin = new Vector2(0f, 1f);
            rt.anchorMax = new Vector2(1f, 1f);
            rt.pivot = new Vector2(0.5f, 1f);
            ...
            var layout = backgroundList.GetComponent<GridLayoutGroup>();
            if (layout == null) return;
            layout.padding = new RectOffset(8, 8, 18, 20);
            layout.cellSize = new Vector2(400f, 460f);
            layout.spacing = new Vector2(12f, 16f);
        ...
        {
            if (backgroundPage == null || backgroundList == null) return;

            var pageRt = backgroundPage.transform as RectTransform;
            var contentRt = backgroundList as RectTransform;
            if (pageRt == null || contentRt == null) return;

            var image = backgroundPage.GetComponent<Image>();
            if (image == null) image = backgroundPage.AddComponent<Image>();
            image.color = new Color(1f, 1f, 1f, 0f);
            image.raycastTarget = true;

            var mask = backgroundPage.GetComponent<RectMask2D>();
            if (mask == null) mask = backgroundPage.AddComponent<RectMask2D>();
            mask.padding = new Vector4(0f, 36f, 0f, 0f);

            if (backgroundMessage != null) backgroundMessage.maskable = false;

            var scroll = backgroundPage.GetComponent<ScrollRect>();
            if (scroll == null) scroll = backgroundPage.AddComponent<ScrollRect>();
            scroll.content = contentRt;
            scroll.viewport = pageRt;
            scroll.horizontal = false;
            scroll.vertical = true;
            ...
            var fitter = backgroundList.GetComponent<ContentSizeFitter>();
            if (fitter == null) fitter = backgroundList.gameObject.AddComponent<ContentSizeFitter>();
            fitter.horizontalFit = ContentSizeFitter.FitMode.Unconstrained;
            fitter.verticalFit = ContentSizeFitter.FitMode.PreferredSize;
        }
        ...
        private static void PlaceTab(Button button, float x)
        {
            ...
            PlaceRect(button.transform as RectTransform, new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
                new Vector2(0.5f, 0.5f), new Vector2(x, 0f), new Vector2(236f, 100f));
        }
        ...
        private void BuildMissingHierarchy()
        {
            ...
            if (tabs == null)
            {
                ...
                PlaceRect(tabsGo.GetComponent<RectTransform>(), new Vector2(0.5f, 1f), new Vector2(0.5f, 1f),
                    new Vector2(0.5f, 1f), new Vector2(0f, -250f), new Vector2(760f, 110f));
            ...
            if (tabs.Find("CharacterTab") == null)
                characterTab = CreateFaceButton(tabs, "CharacterTab",
                    ...
                    new Vector2(-248f, 0f), new Vector2(236f, 100f), ResolveBtn(), GameTexts.TabCharacter, 28);

            if (tabs.Find("BackgroundTab") == null)
                backgroundTab = CreateFaceButton(tabs, "BackgroundTab",
                    new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
                    new Vector2(0f, 0f), new Vector2(236f, 100f), ResolveBtn(), GameTexts.TabBackground, 28);

            if (tabs.Find("GoldTab") == null)
                goldTab = CreateFaceButton(tabs, "GoldTab",
                    new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
                    new Vector2(248f, 0f), new Vector2(236f, 100f), ResolveBtn(), GameTexts.TabGold, 28);
