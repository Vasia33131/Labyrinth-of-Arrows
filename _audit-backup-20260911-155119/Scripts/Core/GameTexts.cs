...
namespace Unpuzzle
{
    ...
    public static class GameTexts
    {
        ...
        /// <summary>Название игры. Должно совпадать с черновиком в консоли Яндекс Игр.</summary>
        ...
        public static string WatchAdForSkin => Pick("Смотреть рекламу — скин", "Watch ad — skin");

        public static string BattlePassLocked => Pick("Доступно в боевом пропуске", "Unlocked in the battle pass");
        ...
        public static string WatchAdToUnlock => Pick("Смотри рекламу, чтобы открыть", "Watch an ad to unlock");
            ...
            switch (scheme)
            {
                ...
                case SchoolTutorialScheme.Briefcase:
                    return IsRussian
                        ? new[]
                        {
                            "Портфель занимает две клетки и тоже перекрывает путь. Тапом его не выгнать с поля.",
                            "Тап по портфелю сдвигает его на одну клетку вдоль своей оси, если место свободно.",
                            "Отодвинь портфель с чужого пути, но не загороди другую стрелку."
                        }
                        : new[]
                        {
                            "The briefcase covers two cells and blocks the path too. Tapping will not push it off the board.",
                            "A tap slides the briefcase one cell along its own axis, if that cell is free.",
