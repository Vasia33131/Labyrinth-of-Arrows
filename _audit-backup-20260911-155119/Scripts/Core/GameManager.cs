...
namespace Unpuzzle
{
    public class GameManager : MonoBehaviour
    {
        ...
        private bool ShouldHoldSchoolTutorial()
        {
            ...
            return IsSchoolMode && !PlayProgress.SchoolTutorialSeen;
        }
        ...
        /// <summary>Тап по портфелю: один шаг + или − по торцу. Не ход, указку не крутит.</summary>
        public void OnBackpackTapped(BackpackController backpack, Vector3 worldTap)
        {
            if (SchoolTutorialPanel.IsOpen) return;
            if (IsIntroPlaying) return;
            if (IsBlockedBumpPlaying) return;
            ...
            LevelSnapshot snapshot = LevelSnapshot.Capture(allArrows, allBooks, allPointers, allBackpacks, MovesUsed);
            if (backpack.TrySlide(Occupancy, worldTap))
            {
                undoSnapshot = snapshot;
