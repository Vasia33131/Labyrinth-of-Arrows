...
namespace Unpuzzle
{
    ...
    /// <summary>
    /// Fullscreen раз в 90 с и только на неигровом клике (п. 4.4): следующий уровень,
    /// рестарт после победы/поражения, вход в уровень из меню, меню, магазин.
    /// Вызов SDK сразу в onClick, без заставки «реклама через 2» и без смены экрана
    /// до ролика — задержка не больше 0,33 с.
    /// Во время хода рекламу не ставим: уровни короткие, это было бы «в перерез».
    /// </summary>
    public static class InterstitialAds
    {
        ...
        public static bool TryShow(InterstitialGate gate)
        {
            ...
            if (gate == InterstitialGate.Shop && ShopPanel.IsOpen) return false;

            // SDK молча проглатывает вызов, если его собственный таймер ещё идёт.
            // Тогда onOpen/onClose не будет — не держим игрока и не стартуем уровень под ролик.
#if UNITY_EDITOR
        ...
        private static bool ShouldSkipPlay()
        {
            ...
            if (YG2.isFirstGameSession) return true;
            if (PlayProgress.TutorialReplayPending) return true;
            if (PlayProgress.IsSchool)
                return !PlayProgress.SchoolTutorialSeen;

            return PlayProgress.CampaignLevel <= GameConstants.InterstitialSkipPlayThroughLevel;
        }
