...
namespace Unpuzzle
{
        ...
        IInitializePotentialDragHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollHandler
    {
        ...
        /// <summary>
        /// Клетка трека и колонка были 92×80 — на телефоне 412 CSS px это 32×28 px,
        /// а клик по клетке забирает награду (см. BattlePassSlotView.OnPointerClick).
        /// 130 логических единиц ≈ 45 CSS px. Ширину колонки держим равной высоте клетки,
        /// иначе квадрат награды обрезается AspectRatioFitter'ом.
        /// </summary>
        public const float SlotSize = 130f;
        ...
        /// <summary>
        /// Единственное место, где скрипт трогает размеры: клетки трека заданы через
        /// LayoutElement в 20 колонках, и править их по сцене руками нереально.
        /// Позиции и иерархия остаются авторскими.
        /// </summary>
        private void ApplyTapSizes()
        {
            if (trackContent == null) return;

            for (int i = 0; i < trackContent.childCount; i++)
            {
                Transform column = trackContent.GetChild(i);
                if (column == null) continue;

                var columnLayout = column.GetComponent<LayoutElement>();
                if (columnLayout != null)
                {
                    columnLayout.preferredWidth = SlotSize;
                    columnLayout.minWidth = SlotSize;
                }

                ResizeSlot(column.Find("Free"));
                ResizeSlot(column.Find("Premium"));
            }
        }

        private static void ResizeSlot(Transform slot)
        {
            if (slot == null) return;
        ...
        public void Refresh()
        {
            ...
            if (levelText != null)
                levelText.text = GameTexts.BattlePassLevel(BattlePass.CurrentLevel, BattlePass.LevelCount);

            bool premium = BattlePass.PremiumUnlocked;
            if (purchaseButton != null)
                purchaseButton.gameObject.SetActive(!premium);
            if (purchaseLabel != null && !premium)
                purchaseLabel.text = GameTexts.Price(BattlePass.PremiumPriceGold);
