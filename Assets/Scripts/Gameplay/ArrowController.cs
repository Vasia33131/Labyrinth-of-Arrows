...
namespace Unpuzzle
{
    ...
    public class ArrowController : MonoBehaviour
    {
        ...
        private void EnsureBookIcon()
        {
            ...
            Vector3 parentLossy = bookIcon.transform.parent != null
                ? bookIcon.transform.parent.lossyScale
                : Vector3.one;
            float parentX = Mathf.Abs(parentLossy.x);
            float parentY = Mathf.Abs(parentLossy.y);
            if (parentX < 0.0001f) parentX = 1f;
            ...
            bookIcon.transform.localScale = new Vector3(
                world / (parentX * spriteSize),
                world / (parentY * spriteSize),
                1f);
        ...
        public void SetFlightPose(Vector3 worldHead, Vector3 tangent, float scale)
        {
            ...
            if (headRenderer != null)
            {
                headRenderer.transform.localScale = CurrentHeadLocalScale(scale);
            ...
            float width = CurrentPathWidth(Mathf.Max(0.15f, scale));
        ...
        private void PlaceHead(Vector3 worldPosition, Vector3 tangent)
        {
            ...
            headRenderer.transform.localScale = CurrentHeadLocalScale();
        ...
        /// <summary>
        /// Телефон + портрет: шире наконечник и тело. ПК и ландшафт — как задано константами.
        /// </summary>
        public static bool UseMobileVisual
        {
            get
            {
        ...
        public static float VisualScale =>
            UseMobileVisual ? GameConstants.MobileArrowVisualScale : 1f;

        /// <summary>
        /// Попадание в визуал стрелки (линия + наконечник). Клетки occupancy не меняет.
        /// На мобилке хитбокс чуть шире линии — палец цепляет выступ головы в пустую клетку.
        /// </summary>
        public bool TryHitVisual(Vector3 world, out float distance)
        {
            distance = float.MaxValue;
            if (IsRemoving && !IsPlayingBlockedBump) return false;

            Vector2 point = world;
            float pathDist = DistanceToRenderedPath(point);
            float headDist = DistanceToHead(world);
            distance = Mathf.Min(pathDist, headDist);

            float pad = UseMobileVisual ? cellSize * 0.16f : 0f;
            float pathHalf = CurrentPathWidth() * 0.5f + pad;
            return pathDist <= pathHalf || headDist <= pad;
        }

        private float DistanceToRenderedPath(Vector2 world)
        {
            if (pathRenderer != null && pathRenderer.positionCount >= 2)
            {
                float best = float.MaxValue;
                bool worldSpace = pathRenderer.useWorldSpace;
                Transform space = pathRenderer.transform;
                Vector3 prev = worldSpace
                    ? pathRenderer.GetPosition(0)
                    : space.TransformPoint(pathRenderer.GetPosition(0));
                for (int i = 1; i < pathRenderer.positionCount; i++)
                {
                    Vector3 next = worldSpace
                        ? pathRenderer.GetPosition(i)
                        : space.TransformPoint(pathRenderer.GetPosition(i));
                    float d = DistanceToSegment(world, prev, next);
                    if (d < best) best = d;
                    prev = next;
                }
            ...
            AppendWorldVisualPoints(points);
            if (points.Count < 2) return float.MaxValue;

            float fallback = float.MaxValue;
            for (int i = 1; i < points.Count; i++)
            {
                float d = DistanceToSegment(world, points[i - 1], points[i]);
        ...
        private float DistanceToHead(Vector3 world)
        {
            if (headRenderer != null && headRenderer.enabled && headRenderer.sprite != null)
            {
                Vector3 closest = headRenderer.bounds.ClosestPoint(world);
                closest.z = 0f;
                world.z = 0f;
                return Vector3.Distance(world, closest);
            }

            return float.MaxValue;
        }

        private static float DistanceToSegment(Vector2 p, Vector3 a, Vector3 b)
        {
            Vector2 ab = new Vector2(b.x - a.x, b.y - a.y);
            Vector2 ap = new Vector2(p.x - a.x, p.y - a.y);
            float abSqr = ab.sqrMagnitude;
            float t = abSqr > 1e-8f ? Mathf.Clamp01(Vector2.Dot(ap, ab) / abSqr) : 0f;
            Vector2 closest = new Vector2(a.x, a.y) + ab * t;
            return Vector2.Distance(p, closest);
        }

        private float CurrentPathWidth(float extra = 1f)
        {
            float width = Mathf.Clamp(pathWidth, GameConstants.PathWidthMinInCells, GameConstants.PathWidthMaxInCells);
            return width * cellSize * VisualScale * Mathf.Max(0.05f, extra);
        }

        private Vector3 CurrentHeadLocalScale(float extra = 1f)
        {
            float s = CurrentHeadScale(extra);
            return new Vector3(s * VisualScale, s, 1f);
        }
        ...
        private void OnValidate()
        {
            ...
            // SpriteRenderer.sprite шлёт SendMessage (OnSpriteTilingPropertyChange) —
            // в OnValidate это запрещено, поэтому превью откладываем на следующий тик.
